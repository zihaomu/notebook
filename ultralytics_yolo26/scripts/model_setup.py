#!/usr/bin/env python3
"""Download YOLO/Qwen assets and compile a target-specific MIGraphX cache."""
...
DEFAULT_YOLO_ASSET_URL = (
    "https://gh-test.anruicloud.com/ultralytics/assets/releases/download/"
    "v8.4.0/{}"
)
YOLO_CHECKPOINT_URL_ENV = "YOLO_CHECKPOINT_URL"
YOLO_ONNX_URL_ENV = "YOLO_ONNX_URL"
...
class ModelSpec:
    ...
    verify_existing_sha256: bool = False


YOLO_CHECKPOINT = ModelSpec(
    "yolo26x.pt",
    118_667_365,
    "9fdd44a31c504547ffb81d2c6d9e6dac3493c8eaa8b0398d3f43bae6c7003e92",
    "ultralytics/assets@v8.4.0",
    url=DEFAULT_YOLO_ASSET_URL.format("yolo26x.pt"),
    url_environment=YOLO_CHECKPOINT_URL_ENV,
...
YOLO_ONNX = ModelSpec(
    ...
    "ultralytics/assets@v8.4.0",
    url=DEFAULT_YOLO_ASSET_URL.format("yolo26x.onnx"),
    url_environment=YOLO_ONNX_URL_ENV,
    verify_existing_sha256=True,
)
QWEN_MODELS = (
    ModelSpec(
...
DOWNLOAD_MODELS = (YOLO_CHECKPOINT, YOLO_ONNX, *QWEN_MODELS)
...
def _basic_status(path: Path, spec: ModelSpec) -> tuple[bool, str]:
    ...
    if spec.verify_existing_sha256 and spec.sha256:
        actual_hash = _sha256(path)
        if actual_hash != spec.sha256:
            return False, f"SHA-256 mismatch: {actual_hash} != {spec.sha256}"
    return True, "ready"
...
def _download_url(spec: ModelSpec, endpoint: str) -> str:
    if spec.url:
        return os.environ.get(spec.url_environment, spec.url) if spec.url_environment else spec.url
...
def download_model(
    ...
    """Download with automatic Range resume, strict validation, and atomic replace."""
    if spec.size is None:
        raise ValueError(f"{spec.filename} is generated locally, not downloaded")
    directory = Path(model_dir or env.MODELS).resolve()
    ...
    attempts = max_attempts or int(os.environ.get("MODEL_DOWNLOAD_ATTEMPTS", "8"))
    if attempts < 1:
        raise ValueError("MODEL_DOWNLOAD_ATTEMPTS must be at least 1")
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            with requests.get(
                ...
                print(
                    f"[models] HTTP {response.status_code}; attempt={attempt}/{attempts}; "
            ...
            if attempt == attempts:
                raise RuntimeError(
                    f"Download failed after {attempts} attempts for {spec.filename}"
                ) from error
            saved = partial.stat().st_size if partial.exists() else 0
            delay = min(2 ** (attempt - 1), 8)
            print(
                f"[models] {spec.filename}: {type(error).__name__}; "
                f"saved={saved:,} bytes; retrying in {delay}s"
            )
            time.sleep(delay)
            continue
        ...
        if actual_size < spec.size:
            if attempt == attempts:
                raise RuntimeError(
                    f"Incomplete download for {spec.filename}: "
                    f"{actual_size:,} != {spec.size:,} bytes"
                )
            print(
                f"[models] {spec.filename}: incomplete response; "
                f"saved={actual_size:,} bytes; resuming"
            )
            continue
        if actual_size > spec.size:
            partial.unlink()
            raise RuntimeError(
                f"Oversized download for {spec.filename}: "
                f"{actual_size:,} != {spec.size:,} bytes"
            )
