...
> 文档状态：实施设计与复现手册，尚未在目标 395 台式机执行
>
> 编写日期：2026-09-08
>
> 源项目：`/home/zihaomu/bigssd/notebook/ultralytics_yolo26`
>
...
当前 release-lock 源码基线包含云端 finite-video pipeline，**尚未包含**
...
2. **最终低延迟本地 pipeline**：按第 9 节新增 latest-frame 调度、可复用上传 buffer、
...
| 优先级 | 资源 | 获取位置 | 固定身份或用途 |
|---|---|---|---|
| 必需 | AMD Linux 驱动 | `https://www.amd.com/en/support/download/linux-drivers.html` | 选择与 Ubuntu 24.04、395 匹配的驱动 |
...
本工作区已有一套**仅用于 gfx1151 原生工具链参考**的 ROCm 7.14 archive：
...
该 archive 已在另一项 395 llama.cpp/GGUF 工作负载中验证，但没有替本项目验证
PyTorch、OpenCV HIP 或 ORT-MIGraphX。优先采用 AMD 当前兼容矩阵给出的完整支持组合；
只有需要 side-by-side 工具链时才使用该 archive，并保持独立前缀。
...
```text
1. M0：确认 gfx1151、kernel、/dev/kfd、摄像头和用户组
2. 安装 AMD 当前支持的 ROCm + PyTorch 组合
3. clone notebook，checkout d9d32c37...
4. 创建 .venv，安装 pinned Ultralytics fork 和普通 opencv-python
5. 只下载并校验 yolo26x.pt
6. ffplay 验证 /dev/video0
7. yolo predict source=0 验证 PyTorch GPU 摄像头基线
8. 按第 9 节实现 latest-frame 低延迟入口并做 30 分钟 Gate
9. 需要时再做 OpenCV HIP、MIGraphX、VA-API、Qwen
```
...
flowchart LR
    ...
    LQ --> UP[Reusable pinned staging + H2D]
    UP --> PRE[OpenCV HIP preprocess]
    PRE --> YOLO[Ultralytics PyTorch default / ORT MIGraphX experimental]
    YOLO --> NMS[OpenCV GPU NMS]
    NMS --> META[Compact detections D2H]
    META --> DISP[CPU overlay on original camera frame]
    DISP --> WIN[Local live window]

    UP -. optional .-> REC[HIP overlay + VAAPI recorder]
...
第一版实时路径保留摄像头采集得到的 host BGR frame，用于窗口显示；同一 frame 通过固定 staging buffer 上传 GPU 做推理。检测后只下载 compact boxes/classes/scores，在原始 host frame 上绘制。这样没有额外完整帧 D2H，但仍存在摄像头帧的必需 H2D。
...
ROCm 使用一套完整、相互匹配的版本。目标建议从 **ROCm 7.14.0 + Python 3.12 + 原生 `gfx1151` wheel/toolchain** 起步。不要混装来自 7.2、7.14 和系统仓库的不同 `libhsa-runtime64`、hipBLAS、MIGraphX 或 PyTorch。

如果目标机已经有经过验证的 ROCm 7.2.1 gfx1151 环境，可以作为备选复现线，但必须单独记录，不能与 7.14 工件混用。

本工作区已有另一个 395 项目在 ROCm 7.14.0、`gfx1151`、Python 3.12 上完成 llama.cpp/GGUF 范围验证；这只证明该基础 toolchain 可用，**不证明**本 YOLO26 所需的 ORT-MIGraphX、OpenCV HIP 和摄像头组合已经通过。本文仍要求逐 Gate 在目标机验证。
...
```bash
mkdir -p ~/ultralytics-yolo26-bringup/logs
{
  date -Is
  uname -a
  lscpu
...
} |& tee ~/ultralytics-yolo26-bringup/logs/00-host-inventory.log
```
...
/dev/video*          = 摄像头存在
vainfo               = 能列出 H.264 encode profile（M4 录制需要）
```
...
若 `rocminfo` 不是 `gfx1151`、kernel 低于要求、或 `/dev/kfd` 不可访问，M0 失败，不能进入模型安装。

## 5. M1：原生 ROCm、Python 与摄像头基线

### 5.1 安装系统依赖

先打开第 1.3 节列出的兼容矩阵和 Linux driver 页面，选择 AMD 当前明确支持
Ryzen AI MAX+ 395、Ubuntu 24.04 和 PyTorch 的同一 release。下载安装包后才运行：

```bash
sudo amdgpu-install -y --usecase=graphics,rocm
sudo apt install -y \
  build-essential cmake ninja-build git git-lfs pkg-config curl rsync \
...
```bash
export ROCM_ARCHIVE=/tmp/therock-dist-linux-gfx1151-7.14.0.tar.gz
export ROCM_PATH="$HOME/rocm-7.14.0"
curl -fL --retry 5 \
  -o "$ROCM_ARCHIVE" \
...
只选一种用户态 ROCm 路线。不要同时把 `/opt/rocm/lib` 和 `$HOME/rocm-7.14.0/lib`
...
dpkg -l | grep -E 'amdgpu|rocm|migraphx' \
  | tee ~/ultralytics-yolo26-bringup/logs/01-rocm-packages.log
"${ROCM_PATH:-/opt/rocm}/bin/hipcc" --version \
  | tee ~/ultralytics-yolo26-bringup/logs/01-hipcc.log
```

### 5.2 创建独立 Python 环境

```bash
mkdir -p ~/ultralytics-yolo26-native
cd ~/ultralytics-yolo26-native
...
PyTorch 必须使用 AMD 当前兼容矩阵为 `gfx1151` 给出的命令，不使用通用 PyPI CPU/CUDA
wheel。安装命令随 ROCm release 变化，因此先在兼容矩阵确认，再把实际命令、wheel URL
和 SHA 写入 `native-lock/python-wheels.txt`。

若官方当前页面暂时没有可用命令，可用下面的历史 gfx1151 pin 做**隔离探针**，但它不是
本文默认交付承诺，且不能与不同 release 的 native extension 混用：
...
将 `pip freeze` 和 wheel 下载 URL/哈希保存到 `native-lock/`；初次不要凭 W7900 容器版本
猜测兼容组合。
...
优先选择原生支持的固定模式，例如 1280x720@30。先验证不经过 AI 的采集：
...
print(frame.shape, cap.get(cv2.CAP_PROP_FPS))
cap.release()
```
...
目标机检出含 release lock 的仓库版本：

```bash
cd ~/ultralytics-yolo26-native
git clone https://github.com/zihaomu/notebook.git notebook
cd notebook
git checkout d9d32c37a29272540937d3ee02b3f8f709046464
cd ultralytics_yolo26
source ~/ultralytics-yolo26-native/.venv/bin/activate
```
...
```bash
rsync -avP \
  user@source:/home/zihaomu/bigssd/notebook/ultralytics_yolo26/models/yolo26x.pt \
  models/
```
...
的 compact detections。现有 `src/detector.py::UltralyticsYOLODetector` 是
...
4. 应用 `docker/patches/ultralytics-migraphx-iobinding.patch`；
...
git clone https://github.com/zihaomu/ultralytics.git third_party/ultralytics
git -C third_party/ultralytics checkout 34e213ca3ece4c18962f5bb922ec74da0c474d24
git -C third_party/ultralytics apply ../../docker/patches/ultralytics-migraphx-iobinding.patch
python -m pip install --no-deps --no-build-isolation -e third_party/ultralytics
```
...
```bash
rm -rf models/ort-migraphx-cache/gfx1151-bringup
...
```bash
source .venv/bin/activate
export OPENCV_INSTALL="$HOME/opt/opencv5-gfx1151"
...
cmake --build build/opencv-gfx1151 -j"$(nproc)"
cmake --install build/opencv-gfx1151
```

如果还需要用 `data/sidewalk.mp4` 跑云端 finite-video 回归，再安装与 ROCm 版本匹配的 rocDecode/rocPyDecode，并单独建立 `WITH_ROCDECODE=ON` build。不要让这个可选依赖阻塞摄像头 MVP。
...
再执行 standalone probe。若 VA-API 驱动不支持所需 surface/encode，M4 录制失败，但不阻塞 M3 实时窗口。
...
云端 `src/pipeline.py` 和 `RocDecodeReader` 保持不变，用录制文件继续做回归。
...
- 后端固定 `cv2.CAP_V4L2`；
- 显式设置 device、width、height、FPS、FOURCC；
- `CAP_PROP_BUFFERSIZE=1`；
- 每帧记录 `capture_seq` 和 `time.monotonic_ns()`；
- 只保存最新一帧；新帧覆盖未消费旧帧；
- 连续 N 次读取失败后关闭设备并报错；
- `stop()` 可在 SIGINT 下有界退出。
...
while not stop_requested:
    frame = latest_slot.consume_after(last_sequence, timeout=1.0)
    last_sequence = frame.sequence
    if backend == "pytorch":
        # New class: owns YOLO(yolo26x.pt) and accepts host BGR.
        detections = pytorch_detector.detect_bgr(frame.bgr)
    else:
        # Existing optimized Route M: RGB GPU -> fixed BCHW -> ONNX/MIGraphX.
    ...
    preview = draw_on_host_frame(frame.bgr, detections, latest_caption)
    display(preview)
```
...
注意：当前发布源码还没有 `scripts/run_camera.py`。按 9.1-9.4 实现并通过 replay tests 后，
默认 PyTorch 命令应为：
...
Qwen 不是实时 YOLO MVP 的一部分。先在 395 上原生构建 llama.cpp：
...
cmake -S third_party/llama.cpp -B third_party/llama.cpp/build-gfx1151 \
  ...
  -DCMAKE_BUILD_TYPE=Release
cmake --build third_party/llama.cpp/build-gfx1151 -j"$(nproc)" \
  --target llama-server
```
...
第一轮 1280x720@30、YOLO26x、VLM off：

- 连续运行 30 分钟；
- 无 crash、GPU reset、camera reopen loop；
- 队列深度不增长；
- capture-to-display p95 <= 150 ms；
- 处理 FPS 目标 >= 25；若 YOLO26x 达不到，先报告真实结果，再评估较小模型或隔帧推理；
...
### M2：OpenCV HIP + 可选 ORT MIGraphX

- OpenCV HIP external pointer/NMS/resize gate 通过；
- PyTorch GPU 实时路径继续可用；
- 若启用 Route M：MIGraphX provider first、GPU I/O Binding/指针复用，并生成 `gfx1151` cache；
- Route M 失败不允许静默切 CPU，也不阻塞 PyTorch MVP。
...
只有前一 Gate 通过才进入下一阶段。M2 阻塞时，M1 PyTorch 实时版仍可作为可用交付，但必须明确 backend 不同。
...
摄像头硬件难以放进 CI，因此增加 replay mode：读取固定 `data/sidewalk.mp4`，但走 `LatestFrameSlot` 和 realtime scheduler，用它覆盖并发、丢帧、退出和 metrics 单测；真实 camera gate 在 395 本机执行并保存 artifacts。
...
```bash
FAIL_DIR="logs/failure-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$FAIL_DIR"
{
  date -Is
  uname -a
...
4. **本地代码回滚到发布源码**：使用独立 worktree，不破坏当前改动：

   ```bash
   cd ~/ultralytics-yolo26-native/notebook
   git worktree add ../notebook-release d9d32c37a29272540937d3ee02b3f8f709046464
   ```

5. **ROCm side-by-side 回滚**：开启新 shell并取消 `$HOME/rocm-7.14.0` 的
   `PATH/LD_LIBRARY_PATH`；不要在运行任务时删除系统驱动。若是 kernel/driver 回归，
   从启动菜单选择上一条已知可用 kernel，并保留 M0 inventory 做对照。

任何回滚层只要恢复 M1 的 PyTorch 摄像头基线，就可以继续现场使用；不要因为 Route M、
...
```text
[ ] 确认型号为 AMD Ryzen AI MAX+ 395（含 PRO 395）/ Radeon 8060S
[ ] rocminfo 确认 gfx1151
[ ] kernel 版本达到 AMD 当前文档要求
[ ] render/video 用户组生效
[ ] TTM/GTT 配置与系统内存记录完成
...
[ ] Route M：MIGraphX provider first、GPU I/O Binding、gfx1151 cache identity
[ ] OpenCV HIP external-pointer/NMS/resize Gate
