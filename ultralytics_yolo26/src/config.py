...
PROJECT_ROOT = Path(os.environ.get(
    "ULTRALYTICS_YOLO26_ROOT",
    os.environ.get("OPENCV_AMD_END2END_ROOT", Path(__file__).resolve().parents[1]),
)).resolve()
MODEL_DIR = Path(os.environ.get(
    "ULTRALYTICS_YOLO26_MODEL_DIR",
    os.environ.get("OPENCV_AMD_END2END_MODEL_DIR", PROJECT_ROOT / "models"),
)).resolve()
YOLO_MODEL_PATH = str(MODEL_DIR / "yolo26x.onnx")
MIGRAPHX_CACHE_ROOT = Path(os.environ.get(
    "ULTRALYTICS_MIGRAPHX_CACHE_ROOT", MODEL_DIR / "ort-migraphx-cache"
)).resolve()
