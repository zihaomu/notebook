...
def draw_detections(frame, detections, descriptions=None, names=None):
    ...
    for det in detections:
        ...
        class_names = names if names is not None else config.COCO_CLASSES
        class_name = (
            class_names.get(class_index, f"cls{class_index}")
            if isinstance(class_names, dict)
            else class_names[class_index]
            if class_index < len(class_names)
            else f"cls{class_index}"
        )
...
def draw_scene_panel(frame, descriptions, names=None):
    ...
    for det, desc in descriptions:
        ...
        class_names = names if names is not None else config.COCO_CLASSES
        name = (
            class_names.get(class_id, f"cls{class_id}")
            if isinstance(class_names, dict)
